...
from util.keys import KEY_ARCHIVE_FILES_BROWSER, KEY_PAGE_DOWN, KEY_PAGE_UP, KEY_PLAYER, KEY_ARCHIVE_FILES, \
    KEY_ARCHIVE_TITLE
...
from ui.navigator.archivefiles import ArchiveFilesNavigator
...
class ArchiveFilesBrowserScreen(MenuScreen):
    ...
    def __init__(self, util, listeners):
        ...
        self.total_pages = 0
        self.title = self.config[LABELS][ARCHIVE_FILES]
        ...
        self.navigator = ArchiveFilesNavigator(self.util, self.layout.BOTTOM, listeners, "archive.navigator")
    ...
    def get_page(self):
        ...
        """
        try:
            playlist = self.config[KEY_ARCHIVE_FILES]
        except:
            return None
    ...
    def turn_page(self):
        ...
        self.archive_menu.set_items(d, 0, self.change_file, False)

        id = self.config[ARCHIVE][FILE]

        if not self.config[KEY_ARCHIVE_TITLE]:
            self.screen_title.set_text(self.ARCHIVE_LABEL)
        else:
            self.screen_title.set_text(self.config[KEY_ARCHIVE_TITLE])
    ...
    def change_file(self, state):
        """ Change file

        :param state: state object
        """
        state.source = KEY_ARCHIVE_FILES_BROWSER
