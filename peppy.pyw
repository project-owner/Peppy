...
class Peppy(object):
    ...
    def timer_thread(self):
        ...
        while self.run_timer_thread:
            ...
            if sleep_selected:
                if wake_up_selected and self.player_state == PLAYER_SLEEPING and self.is_time_in_range(current_time,
                                                                                                       wake_up_time):
                    with self.lock:
                        self.player_state = PLAYER_RUNNING
                    self.wake_up()
    ...
    def set_mode(self, state):
        ...
        elif mode == AUDIO_FILES:
            self.go_file_playback(state)
        elif mode == STREAM:
            self.go_stream(state)
        elif mode == AUDIOBOOKS:
            self.go_audiobooks(state)
        elif mode == CD_PLAYER:
    ...
    def go_file_browser(self, state=None):
        ...
        file_browser_screen = FileBrowserScreen(self.util, self.player.get_current_playlist, self.player.load_playlist,
                                                listeners, self.voice_assistant)
    ...
    def go_site_authors_screen(self, ch, f=None):
        ...
        d = self.get_book_menu_settings()
        s = BookAuthor(self.util, listeners, ch, f, self.go_site_books_by_author, self.get_parser(), base_url,
                       self.voice_assistant, d)
        self.screens[name] = s
        self.set_current_screen(name)
    ...
    def go_site_books_by_author(self, state):
        ...
        s = BookAuthorBooks(self.util, listeners, state.name, self.go_site_playback, state.url, parser,
                            self.voice_assistant, d)
        self.screens[name] = s
        self.set_current_screen(name)
    ...
    def go_site_books_by_genre(self, state):
        ...
        s = BookGenreBooks(self.util, listeners, state.name, self.go_site_playback, state.genre, parser,
                           self.voice_assistant, d)
        self.screens[name] = s
        self.set_current_screen(name)
    ...
    def go_podcast_player(self, state):
        ...
        listeners[KEY_SEEK] = self.player.seek
        screen = PodcastPlayerScreen(listeners, self.util, self.player.get_current_playlist, self.voice_assistant,
                                     self.player.stop)
                ...
                if name == KEY_STATIONS:
                    ...
                    if self.cdutil.get_cd_drive_id_by_name(cd_drive_name) != None:
                        ...
                        if file_name and file_name.startswith("cdda:"):
                            ...
                            if s and s.file_name:
                                ...
                                if cd_track_id != self.config[CD_PLAYBACK][
                                    CD_TRACK] or self.current_player_screen != name:
                    ...
                    if f and self.current_audio_file != f or self.current_player_screen != name or state.source == INIT:
                        ...
                        if source == "episode_menu":
                            ...
                            s.podcast_image_url = podcasts_util.summary_cache[s.podcast_url].episodes[
                                0].podcast_image_url
